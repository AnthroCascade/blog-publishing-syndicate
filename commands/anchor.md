...
None. Main-loop only. The Machine produces directly; no Agent dispatch. Deliberately does not follow the coordination syndicate's `orchestrator-preamble.md` — the orchestrator preamble governs forte dispatch through Agent calls, which is not what this command does.
